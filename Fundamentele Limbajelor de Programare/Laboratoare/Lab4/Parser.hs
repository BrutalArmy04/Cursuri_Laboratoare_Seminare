...
parseFirst p s =
  case parse (parseAll (spaces *> p)) s of 
...
spaces = Parser (\cs -> let (sp, rest) = span isSpace cs in [(sp, rest)])
...
identifier = some (sat isAlpha) <* spaces
...
variable = (:) <$> sat isUpper <*> many (sat isAlphaNum) <* spaces
...
-}
pconst :: Parser Expr
pconst = (\val -> App val []) <$> (number <|> atom)
  where
    atom = (:) <$> sat isLower <*> many (sat isAlphaNum) <* spaces
    number = (++) <$> ((sat (== '-') $> "-") <|> pure "") <*> some (sat isDigit) <* spaces
