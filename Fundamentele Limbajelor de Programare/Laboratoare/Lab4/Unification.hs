...
unifyStep (Config [] subst) = Right (Config [] subst)
unifyStep (Config ((e1 :=: e2) : eqs) subst) = case (e1, e2) of
  (Var x, Var y) | x == y -> 
      Right (Config eqs subst) 
  (Var x, e)
      | x `elem` vars e -> 
          Left $ "Cycle on " ++ x
      | otherwise -> 
          let s = singletonSubst x e `asTypeOf` subst
              newEqs = map (eapply s) eqs
              newSubst = extend subst x e
          in Right (Config newEqs newSubst)
          
  (App f es, Var x) -> 
      Right (Config ((Var x :=: App f es) : eqs) subst)
      
  (App f es1, App g es2)
      | f == g && length es1 == length es2 -> 
          Right (Config (zipWith (:=:) es1 es2 ++ eqs) subst)
      | otherwise -> 
          Left $ "Conflict between " ++ f ++ "/" ++ show (length es1) ++ " and " ++ g ++ "/" ++ show (length es2)
