...
def cov_corr(N, X, Z):
    ...
    med_x = np.average(X)
    med_y = np.average(Y)
    cov = 1/(N-1) * np.average((X - med_x)* (Y - med_y))
    corr = cov / math.sqrt((1/(N-1) * np.average((X - med_x)* (X - med_y))) * 1/(N-1) * np.average((Y - med_x)* (Y - med_y)))
    print(cov)
    print(corr)
...
def regression(N, X, Y):
    ...
    Xpatrat = sum(x**2 for x in X)
    Xsum = sum(X)
    Ysum = sum(Y)
    XY = sum(x*y for x in X for y in Y)

    A = np.array([[Xpatrat, Xsum], [Xsum, N]])
    B = np.array([[XY], [Ysum]])
    ...
    return sol[0], sol[1]
    #_, ax = plt.subplots
    #ax.plot([min(X), max(X)], [a min(X) + bfor a in X for b in Y , a max(X) + b for a in X for b in Y])
