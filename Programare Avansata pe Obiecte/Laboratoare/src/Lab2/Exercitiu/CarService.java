//package lab2.laboratory01.exercise;

package lab2.Exercitiu;
...
public class CarService {
    ...
    public void addReview(String carName, String review) {
        for (int i = 0; i < cars.length; i++) {
            ...
            { 
                String[] oldrev = cars[i].getReviews();
                String[] newrev = new String[oldrev.length + 1];
                
                System.arraycopy(oldrev, 0, newrev, 0, oldrev.length);
                newrev[newrev.length - 1] = review;
                cars[i].setReviews(newrev);
                
                System.out.println("Reviewul" + cars[i].getReviews()[oldrev.length] + "a fost adaugat");
